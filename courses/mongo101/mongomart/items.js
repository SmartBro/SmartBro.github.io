...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */
        ...
        this.db.collection('item').aggregate([
            ...
            categories = docs;
            var totalNum = categories.reduce((total, category) => {
                return total + category.num
            }, 0);
            var category = {
                _id: "All",
                num: totalNum
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
         ...
         */
        var pageItems = [];
        var search = category === 'All' ? {} : { category };

        this.db.collection('item')
        .find(search)
        .sort({'_id': 1})
        .skip(page * itemsPerPage)
        .limit(itemsPerPage).toArray((err, docs) => {
            pageItems = docs;
    ...
    this.getNumItems = function(category, callback) {
        ...
        .aggregate([
            {$match: search},
            {$group: {_id: 'category', 'numItems': {$sum: 1}}}
        ]).toArray((err, docs) => {
            callback(docs[0].numItems);
        });
    }
    ...
    this.getItem = function(itemId, callback) {
        ...
        this.db.collection('item')
        .find({ '_id': itemId }).toArray((err, docs) => {
